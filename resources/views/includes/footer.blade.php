<footer class="bg-primary p-4 mt-100">
    <div class="container">
        <div class="row">
            <div class="col-md-1 d-none d-md-block">
                <img src="{{url('main-assets/images/logo-jabar.png') }}" style="width: 75%" class="img-fluid" />
            </div>
            <div class="col-md-5">
                <p class="footer-header">Dinas Pendidikan Provinsi Jawa Barat</p>
                <hr />
                <p class="footer-core-info">
                    <b>Layanan Contact Center Disdik</b> <br />
                    Call Center : (022) 8827728 <br />
                    Formulir Pengaduan — <a href="http://disdik.jabarprov.go.id/complaintform"
                        class="text-white ">Link</a> <br />
                    Live Chat — <a href="#" class="text-white">Link</a> <br /><br />
                    Jam operasional Contact Center : <br />
                    Hari Senin — Minggu, Pukul 08.00 - 20.00 WIB
                </p>
                <hr />
                <p>
                    <a href="https://www.instagram.com/disdikjabar/" class="text-decoration-none text-white"> <i
                            class="lni lni-instagram"></i></a>
                    &nbsp;
                    <a href="https://www.facebook.com/disdik.jabarprov/" class="text-decoration-none text-white"><i
                            class="lni lni-facebook-original"></i></a>
                    &nbsp;
                    <a href="https://twitter.com/disdik_jabar" class="text-decoration-none text-white"><i
                            class="lni lni-twitter-original"></i></a>
                </p>
            </div>
            <div class="col-md-4 ms-auto d-none d-md-block">
                <ul class="footer-link" style="list-style: none">
                    <a class="text-white" href="{{url('/')}}">
                        <li>Beranda</li>
                    </a>
                    <a href="{{url('/tentang-kami')}}" class="text-white">
                        <li class="mt-3">Tentang Kami</li>
                    </a>
                    <a href="{{url('/laporan-sekolah')}}" class="text-white">
                        <li class="mt-3">Laporan Sekolah</li>
                    </a>
                    <a href="{{url('/faq')}}" class="text-white">
                        <li class="mt-3">FAQ</li>
                    </a>
                    <a href="{{url('/kontak')}}" class="text-white">
                        <li class="mt-3">Hubungi Kami</li>
                    </a>
                    <a href="{{url('/login')}}" class="text-white">
                        <li class="mt-3">Masuk — Login</li>
                    </a>
                </ul>
            </div>
        </div>
    </div>
</footer>